...
char* indexes(const char* path)
{
    // TODO
    struct stat file;
    // add index.php/html to path
    ///index. = 6 + php = 3 / + html = 4 + \0 = 1
    size_t len = strlen(path);
    char* html = malloc(len + 7 + 4 + 1);
    char* php = malloc(len + 7 + 3 + 1);
    char* htmladd = "/index.html";
    char* phpadd = "/index.php";
    strcpy(html, path);
    strcat(html, htmladd);
    strcpy(php, path);
    strcat(php, phpadd);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file)) {
        tempFile[i] = (char) c;
        i++;
        tempFile = (char*) realloc(tempFile, sizeof(char) * (i+1));
    }
    *length = i;
    *content = tempFile;
    return true;
...
const char* lookup(const char* path)
{
    ...
    if (strcasecmp(end, ".css") == 0) {
        return "text/css";
    } else if (strcasecmp(end, ".html") == 0) {
        return "text/html";
    } else if (strcasecmp(end, ".gif") == 0) {
        return "image/gif";
    } else if (strcasecmp(end, ".ico") == 0) {
        return "image/x-icon";
    } else if (strcasecmp(end, ".jpg") == 0) {
        return "image/jpeg";
    } else if (strcasecmp(end, ".js") == 0) {
        return "text/javascript";
    } else if (strcasecmp(end, ".php") == 0) {
        return "text/x-php";
    } else if (strcasecmp(end, ".png") == 0) {
        return "image/png";
    } else {
        return NULL;
    }
...
{
    // TODO
    char* lineCopy = malloc(sizeof(char) * 1024);
    strcpy(lineCopy, line);
    char* words = strtok(lineCopy, " ");
    
    // FOR GET/POST
    char* method = malloc(sizeof(char) * 1024);
    strcpy(method, words);
    words = strtok(NULL, " ");
    
    // FOR LOCATION OF REQUEST
    char* request = malloc(sizeof(char) * 1024);
    strcpy(request, words);
    words = strtok(NULL, " ");
    
    // FOR HTML TYPE
    char* http = malloc(sizeof(char) * 1024);
    strcpy(http, words);
    ...
    if (strcmp(http, "HTTP/1.1") != 0 && strcmp(http, "HTTP/1.1\r\n") != 0) {
        error(505);
        return false;
    }
    ...
    if (strstr(request, "?") != NULL) {
        char* tempq = strstr(request, "?") + 1;
        strcpy(query, tempq);
        //printf("%s", tempq);
        //getchar();
        /*query = strstr(request, "?");*/
        int num = strstr(request, "?") - request;
        strncpy(abs_path, request, num);
    } else {
        strcpy(query, "");
